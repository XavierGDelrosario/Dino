...
| Reader-side lemma | — | `lemmaEn.ts` covers irregulars (incl. the WordNet long tail via `irregularsEn.generated.ts`), plurals and possessives; regular **-ing/-ed/-es** the exception list does not name are deliberately unhandled (no verifier — a wrong lemma resolves the word to something else). *running* → *run* still needs the tagger. |
