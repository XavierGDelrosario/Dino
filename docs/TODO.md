...
- **Generation rules:** demonstrate THAT sense, not the word generally · **match the word's own level and register** — no single supporting word *harder* than the target (that makes you look up two things), but same-level words are fine and make the sentence authentic · natural JA, not English word order · short enough for a hover card.
  - ⚠️ Do **not** re-impose "every supporting word more common than the target" (the first draft of this rule). Tested 2026-08-01 on hard vocabulary: it rejects 「この部屋は書斎と客間を兼ねている」 because 書斎/客間 aren't easier than 兼ねる — yet that is the natural sentence, and nobody meeting 兼ねる is troubled by 書斎. Anyone reading JA definitions is already competent and wants to accelerate.
  - **The JA definition must carry what the English gloss structurally cannot** — collocation, negation habits, what an auxiliary attaches to. 遜色 glossed "inferiority" invites 遜色がある (unnatural); the definition says 多くは「ない」を伴って使う. That usage note *is* the value. At this level the JA definition is the primary field and English is the fallback.
