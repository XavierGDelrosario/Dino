-- =========================================================
-- EN->JA quality: rank verb senses correctly (zero new storage).
-- JMdict glosses verbs in the infinitive ("to do; to carry out", "to eat"), so the
-- EN->JA head-match regex (^do…) never matched them — verbs fell to the mid-gloss
-- tier and ranked by raw frequency, surfacing noise (do -> な instead of する/やる).
-- Allow an optional leading "to " in the head-match (exact + gloss-head + central),
-- so "do" head-matches "to do" -> する/やる/行う, "eat" -> 食べる, etc. Reverse-gloss
-- search over the JMdict data already loaded; no new dictionary, no extra space.
-- Forward-only CREATE OR REPLACE (carries the uk fix from 20260701).
-- =========================================================
