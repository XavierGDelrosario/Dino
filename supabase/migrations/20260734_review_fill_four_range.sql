-- =========================================================
-- review_queue: widen the confidence-4 fill band from 3–4 to 3–8 (product decision).
--
-- Only the conf-4 quota changes. Everything else is 20260733 verbatim: the DUE phase
-- (R ≤ 0.9, the line record_review freezes at — including decayed conf-5 words, which is
-- how a mature word lapses), the conf-5 cameo (1% per fill slot, capped at ONE, never
-- used as padding), and the ≤3 pool taking whatever is left.
--
-- Against the default 20-card session, when nothing is due:
--   confidence 0–3 :  12–17  (the remainder — still the bulk)
--   confidence 4   :  3–8    (15% floor + a uniform draw of up to 25% more)
--   confidence 5   :  0, or 1 as a ~1-in-100-cards cameo
--
-- The wide band is deliberate: a fixed count makes the rotation feel mechanical, and the
-- 4s are where the variety lives. Written as proportions of the fill-slot count, so a
-- shorter session (a filtered sub-list) keeps the same shape rather than collapsing.
-- =========================================================
