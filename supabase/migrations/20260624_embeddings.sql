...
CREATE TABLE IF NOT EXISTS word_embeddings (
  jmdict_entry_id TEXT PRIMARY KEY REFERENCES jmdict_entries(entry_id) ON DELETE CASCADE,
...
DECLARE
  v_target vector(384);
BEGIN
  SELECT we.embedding INTO v_target FROM word_embeddings we WHERE we.jmdict_entry_id = p_entry_id;
