...
TABLES=(users user_limits translation_usage lists user_words list_words review_log)
...
echo "[restore-test] cloning live schema (schema-only) into scratch …"
...
docker exec "$CONTAINER" pg_dump -U postgres -d postgres --schema-only --no-owner --no-privileges -n public \
  | grep -vE '^\\(un)?restrict' \
...
echo "[restore-test] comparing row counts (live vs restored) …"
FAIL=0
printf '  %-22s %10s %10s\n' table live restored
for t in "${TABLES[@]}"; do
  LIVE="$(count postgres "$t")"
