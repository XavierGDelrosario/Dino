...
- **Dictionary source (JMdict) not wired** — `callTranslationProvider` in the edge function is still a throwing stub. Plan: **store JMdict entirely** as the primary source (so every dictionary word is a local hit — this obsoletes any "common words" pre-seed) with **MT as fallback** for words JMdict lacks + the whole-paragraph display translation. The write path is **ready**: `callTranslationProvider` returns a `ProviderResult { translation, inputReading?, translationReading? }` and the handler persists those readings into `words` — so wiring JMdict is *implementing that function* to return the kana reading on the correct side (input vs translation), nothing downstream. Until it's wired, no real senses are produced and `input_reading` / `translation_reading` stay NULL (the read mapping/plumbing already carries them end-to-end). Readings are stored in the source's script (kana for JA, pinyin for ZH); romaji is intentionally not a feature (it'd be a pure display-time `kana→romaji` conversion, no schema impact).
