...
- **Dictionary source (JMdict) not wired** — `callTranslationProvider` in the edge function is still a throwing stub. Plan: **store JMdict entirely** as the primary source (so every dictionary word is a local hit — this obsoletes any "common words" pre-seed) with **MT as fallback** for words JMdict lacks + the whole-paragraph display translation. Until it's wired, no real senses are produced and `input_reading` / `translation_reading` stay NULL (the mapping/plumbing already carries them end-to-end). Readings are stored in the source's script (kana for JA, pinyin for ZH); romaji is intentionally not a feature (it'd be a pure display-time `kana→romaji` conversion, no schema impact).
...
- **Furigana mono-ruby alignment** (polish) — `words` readings are whole-word, so furigana renders as group ruby (reading over the whole term). Per-kanji placement (kana only over kanji, okurigana left plain) needs an alignment layer: the **JmdictFurigana** dataset for stored dictionary words (fits the JMdict store, no analyzer needed), **kuroshiro** for free text. kuromoji alone gives word-level readings, not alignment.
